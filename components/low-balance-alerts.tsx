    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card className="p-4 bg-green-500/10 border-green-500/30 hover:border-green-500/50 transition-colors">
          <div className="flex items-start justify-between">
            ...
            <TrendingDown className="w-6 h-6 text-green-600 dark:text-green-400 mt-1 flex-shrink-0" />
          </div>
        </Card>
      </div>
      ...
      {criticalCount > 0 && (
        <Card className="p-4 bg-red-500/5 border-red-500/30">
          <h3 className="font-semibold text-red-600 dark:text-red-400 mb-3 flex items-center gap-2 text-sm sm:text-base">
          ...
          <div className="space-y-2">
            {criticalAlerts.map((alert, idx) => (
              <div
                key={idx}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-2 bg-background/50 rounded border border-red-500/20"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-xs font-mono text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 underline break-all"
                  >
                    <span className="sm:hidden">{alert.shortAddress}</span>
                    <span className="hidden sm:inline">{alert.address}</span>
      ...
      {warningCount > 0 && (
        <Card className="p-4 bg-yellow-500/5 border-yellow-500/30">
          <h3 className="font-semibold text-yellow-600 dark:text-yellow-400 mb-3 flex items-center gap-2 text-sm sm:text-base">
          ...
          <div className="space-y-2">
            {warnings.map((warning, idx) => (
              <div
                key={idx}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-2 bg-background/50 rounded border border-yellow-500/20"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-xs font-mono text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 underline break-all"
                  >
                    <span className="sm:hidden">{warning.shortAddress}</span>
                    <span className="hidden sm:inline">{warning.address}</span>
                    <ExternalLink className="w-3 h-3 flex-shrink-0" />
                  </a>
      ...
      {healthyCount > 0 && (
        <Card className="p-4 bg-green-500/5 border-green-500/30">
          <h3 className="font-semibold text-green-600 dark:text-green-400 mb-3 flex items-center gap-2 text-sm sm:text-base">
          ...
          <div className="space-y-2">
            {healthy.map((signer, idx) => (
              <div
                key={idx}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-2 bg-background/50 rounded border border-green-500/20"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-xs font-mono text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 underline break-all"
                  >
                    <span className="sm:hidden">{signer.shortAddress}</span>
                    <span className="hidden sm:inline">{signer.address}</span>
                    <ExternalLink className="w-3 h-3 flex-shrink-0" />
                  </a>
